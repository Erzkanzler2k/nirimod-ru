...
- **Nix-пакеты:** при открытии страницы мгновенно показываются ~100 самых популярных пакетов Nixpkgs (встроенный реестр, без ожидания). Полный поиск по Nixpkgs — по запросу: первый запуск прогревает индекс Nix в фоне, дальнейшие поиски занимают около секунды.
...
- **Кэш поиска:** повторные запросы загружаются из локального кэша (`~/.cache/nirimod/`) мгновенно, даже без сети.
