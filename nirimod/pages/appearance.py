...
class AppearancePage(BasePage):
    def build(self) -> Gtk.Widget:
        tb, _, _, content = self._make_toolbar_page("Оформление")
    ...
    def _build_content(self):
        ...
        draw_behind_row = Adw.SwitchRow(
            ...
            subtitle="Устраняет артефакты углов в приложениях без CSD",
        )
        ...
        blur_grp = Adw.PreferencesGroup(
            title="Размытие (глобально)",
            description=(
                "Требуется Niri 26.04 или новее. Задаёт качество размытия и "
                "опциональные правила размытия окон."
            ),
        )
        ...
        blur_enabled_row = Adw.SwitchRow(
            title="Принудительно размывать окна",
            subtitle="Добавляет background-effect { blur true } в глобальное правило окон",
        )
        ...
        focused_blur_row = Adw.SwitchRow(
            title="Оставлять размытыми окна в фокусе",
            subtitle="Добавляет правило для окна в фокусе, принудительно включающее размытие",
        )
        ...
        xray_row = Adw.SwitchRow(
            title="Размытие обоев (Xray)",
            subtitle="Размывать только обои; отключите для обычного размытия фона",
        )
        ...
        opacity_row = Adw.SpinRow(
            title="Непрозрачность окна (1 = не задано)", adjustment=opacity_adj, digits=2
        )
        ...
        border_bg_row = Adw.SwitchRow(
            title="Рисовать границу вместе с фоном",
            subtitle="Отключите, чтобы не было цветов фокуса за полупрозрачными окнами",
        )
        ...
        passes_row = Adw.SpinRow(title="Проходы", adjustment=passes_adj, digits=0)
    ...
    def _build_border_group(
        ...
        off_row = Adw.SwitchRow(title="Включено")
