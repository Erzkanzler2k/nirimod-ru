...
class EnvironmentPage(BasePage):
    def build(self) -> Gtk.Widget:
        tb, header, _, content = self._make_toolbar_page("Переменные окружения")
    ...
    def _rebuild(self):
        ...
        if not entries:
            status = Adw.StatusPage(
                title="Переменные окружения не заданы",
                description="Переменные, заданные здесь, будут применяться к niri и всем запускаемым им процессам.",
            ...
            grp = Adw.PreferencesGroup(
                ...
                description=f"Настроено переменных: {len(entries)}",
            )
            ...
            add_btn = Gtk.Button(label="Добавить ещё одну переменную")
            add_btn.add_css_class("pill")
            add_btn.set_halign(Gtk.Align.CENTER)
    ...
    def _show_dialog(self, node: KdlNode | None, idx: int):
        dialog = Adw.AlertDialog(
            heading="Переменная окружения", body="Задайте переменную окружения в формате key=value."
        )

        key_entry = Adw.EntryRow(title="Имя переменной (напр. QT_QPA_PLATFORM)")
        val_entry = Adw.EntryRow(title="Значение (напр. wayland)")
        if node:
