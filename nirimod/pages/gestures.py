...
_CORNERS = [
    ("top-left", "Верхний левый", "Перемещает курсор в верхний левый угол"),
    ("top-right", "Верхний правый", "Перемещает курсор в верхний правый угол"),
    ("bottom-left", "Нижний левый", "Перемещает курсор в нижний левый угол"),
    ("bottom-right", "Нижний правый", "Перемещает курсор в нижний правый угол"),
]
...
class GesturesPage(BasePage):
    ...
    def _build_content(self):
        ...
        hc_grp = Adw.PreferencesGroup(
            title="Горячие углы",
            description="Открывайте обзор, когда курсор касается угла экрана (niri ≥ 25.05)",
        )
        ...
        hc_expander = Adw.ExpanderRow(
            title="Включить горячие углы",
            subtitle="Разверните, чтобы выбрать активные углы (по умолчанию: верхний левый)",
        )
        ...
        # ── Hotkey Overlay ────────────────────────────────────────────────────
        hko_grp = Adw.PreferencesGroup(title="Оверлей горячих клавиш")
        ...
        skip_row = Adw.SwitchRow(
            ...
            subtitle="Не показывать оверлей горячих клавиш при запуске niri",
        )
        ...
        ss_grp = Adw.PreferencesGroup(
            title="Снимки экрана", description="Шаблон пути для сохранения снимков экрана"
        )
        cur_path = next(
        ...
        ws_shadow_row = Adw.SwitchRow(
            title="Тень рабочих пространств в обзоре",
            subtitle="Показывать тени под рабочими пространствами в режиме обзора",
        )
