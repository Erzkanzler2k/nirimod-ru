...
class LayoutPage(BasePage):
    def build(self) -> Gtk.Widget:
        tb, _, _, content = self._make_toolbar_page("Компоновка")
    ...
    def _build_content(self):
        ...
        basic_grp = Adw.PreferencesGroup(title="Основные")

        gaps_val = int(layout.child_arg("gaps") or 16)
        gaps_adj = Gtk.Adjustment(value=gaps_val, lower=0, upper=200, step_increment=2)
        gaps_row = Adw.SpinRow(title="Отступы окон (px)", adjustment=gaps_adj, digits=0)
        ...
        cfc_row = Adw.ComboRow(title="Центрировать сфокусированную колонку", model=cfc_model)
        ...
        prefer_csd_row = Adw.SwitchRow(
            title="Предпочитать без CSD",
            subtitle="Просить приложения отключить клиентские декорации",
        )
        ...
        pw_grp = Adw.PreferencesGroup(title="Заготовки ширины колонок (пропорции)")
        ...
        add_preset_btn = Gtk.Button(label="Добавить заготовку")
        ...
        struts_grp = Adw.PreferencesGroup(title="Внешние отступы (struts, px)")
