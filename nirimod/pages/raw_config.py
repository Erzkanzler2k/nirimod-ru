...
class RawConfigPage(BasePage):
    def build(self) -> Gtk.Widget:
        ...
        title_label = Gtk.Label(label="Файл конфигурации")
        ...
        self._discard_btn = Gtk.Button(label="Сбросить")
    ...
    def _do_load_file(self, idx: int):
        path = self._current_files[idx]
        text = path.read_text() if path.exists() else f"// Файл не найден: {path}"
    ...
    def _on_reloaded(self, result):
        ...
        if ok:
            ...
            self.show_toast(f"Сохранено, но не удалось перезагрузить: {msg[:80]}", timeout=8)
    ...
    def _confirm_discard_then(self, callback):
        ...
        dialog = Adw.AlertDialog(
            ...
            body="Несохранённые правки этого файла будут потеряны.",
        )
        dialog.add_response("cancel", "Отмена")
        dialog.add_response("discard", "Сбросить")
