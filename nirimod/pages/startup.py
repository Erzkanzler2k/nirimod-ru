...
class StartupPage(BasePage):
    def build(self) -> Gtk.Widget:
        tb, header, _, content = self._make_toolbar_page("Программы автозапуска")
    ...
    def _rebuild(self):
        ...
        if not entries:
            status = Adw.StatusPage(
                ...
                description="Добавленные здесь программы будут запускаться автоматически при старте niri.",
            ...
            grp = Adw.PreferencesGroup(
                title="Программы автозапуска",
                description=f"Программ к запуску: {len(entries)}",
            )
    ...
    def _make_row(self, node: KdlNode, idx: int) -> Adw.ActionRow:
        ...
        subtitle_parts.append(
            "Через shell (spawn-sh-at-startup)" if is_sh else "Запускается напрямую"
        )
    ...
    def _show_dialog(self, node: KdlNode | None, idx: int):
        dialog = Adw.AlertDialog(
            heading="Программа автозапуска",
            body="Введите команду для запуска при старте.",
        )
        cmd_entry = Adw.EntryRow(title="Команда")
        sh_switch = Adw.SwitchRow(title="Использовать shell (spawn-sh-at-startup)")
        delay_adj = Gtk.Adjustment(
            value=0, lower=0, upper=3600, step_increment=1, page_increment=10
        )
        delay_row = Adw.SpinRow(
            title="Задержка",
            subtitle="Секунды ожидания перед запуском",
